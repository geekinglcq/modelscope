...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None, forward_model_kwargs={}):
        if t_continuous.reshape((-1, )).shape[0] == 1:
            t_continuous = t_continuous.expand((x.shape[0]))
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            #import pdb;pdb.set_trace()
            output = model(x, t_input, **forward_model_kwargs)
        else:
            output = model(x, t_input, cond, **forward_model_kwargs)
        if model_type == "noise":
            return output
    ...
    def model_fn(x, t_continuous, model_kwargs=None):
        """
        ...
        if guidance_type == "uncond":
            ...
            if isinstance(model_kwargs, list) and len(model_kwargs) == 2:
                ...
                #import pdb;pdb.set_trace()
                t_input = get_model_input_time(t_continuous)
                print(t_continuous, t_input)
                noise_cond = noise_pred_fn(x, t_input, None, model_kwargs[0])
                noise_uncond = noise_pred_fn(x, t_continuous, None,
                                             model_kwargs[1])
                a = noise_uncond[:, :3]
                b = guidance_scale * (noise_cond[:, :3] - noise_uncond[:, :3])
                return a + b
            elif guidance_scale == 1. or unconditional_condition is None:
                return noise_pred_fn(
                    x, t_continuous, cond=condition, model_kwargs)
            else:
                x_in = torch.cat([x] * 2)
...
class DPM_Solver:
    ...
    def noise_prediction_fn(self, x, t, model_kwargs=None):
        """
        Return the noise prediction model.
        """
        return self.model(x, t, model_kwargs)

    def data_prediction_fn(self, x, t, model_kwargs=None):
        """
        Return the data prediction model (with thresholding).
        """
        noise = self.noise_prediction_fn(x, t, model_kwargs)
        dims = x.dim()
    ...
    def model_fn(self, x, t, model_kwargs=None):
        """
        Convert the model to the noise prediction model or the data prediction model. 
        """
        if self.predict_x0:
            return self.data_prediction_fn(x, t, model_kwargs)
        else:
            return self.noise_prediction_fn(x, t, model_kwargs)
    ...
    def sample(self,
        ...
        if method == 'adaptive':
            ...
            with torch.no_grad():
                ...
                model_prev_list = [self.model_fn(x, vec_t, model_kwargs)]
                t_prev_list = [vec_t]
                ...
                for init_order in range(1, order):
                    ...
                    model_prev_list.append(
                        self.model_fn(x, vec_t, model_kwargs))
                    x = x.float()
